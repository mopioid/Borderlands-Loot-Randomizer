...
class Enemy(Location):
    def enable(self) -> None:
        ...
        if not self.specified_rarities:
            ...
            if self.tags & Tag.RaidEnemy:       self.rarities += (100,100,100,50,50,50)
...
def _pawn_died(caller: UObject, function: UFunction, params: FStruct) -> bool:
    balance: Optional[UObject] = caller.BalanceDefinitionState.BalanceDefinition
    if not balance:
        return True
    ...
    pools = [
        convert_struct(pool) for pool in caller.ItemPoolList
